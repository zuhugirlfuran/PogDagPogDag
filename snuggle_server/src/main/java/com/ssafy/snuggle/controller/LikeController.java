...
public class LikeController {
	...
	@PostMapping("/insert")
	@Operation(summary = "상품의 좋아요를 like 데이터에 추가한다.", description = "userId와 productId가 동일한 like 객체가 이미 존재하면 해당 좋아요를 제거한다. (이때 결과값은 false), "
	...
	@Operation(summary = "좋아요 상태 조회", description = "특정 유저가 특정 상품에 좋아요를 눌렀는지 확인합니다.")
	public Boolean isLiked(@PathVariable String userId, @PathVariable int productId) {
	    return lService.isLiked(userId, productId);
	}
